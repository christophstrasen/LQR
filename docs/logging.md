...
- The shared logger lives in `log.lua`. Levels: `fatal`, `error`, `warn`, `info`, `debug`, `trace`. Default is `warn`, unless `LOG_LEVEL` is set or `DEBUG=1` (implies `debug`).
- Tagging: `local Log = require("log").withTag("join")` then `Log.warn("...")`/`Log.debug("...")`. Use tags per subsystem (e.g., `join`, `viz-hi`, `viz-lo`, `demo`).
