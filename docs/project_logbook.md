...
### Highlights
- **Per-key buffers in the core:** The JoinObservable caches no longer store a single record per key; instead, each side now keeps a small ring buffer of recent records per join key. A shared default `perKeyBufferSize = 10` means the core is non-distinct by default and only becomes “latest-only” when we explicitly set a buffer of size 1. Every removal path now emits on the expired stream: count-window evictions use `reason = "evicted"`, time/predicate windows use `expired_*`, overwrites use `replaced`, and completion/disposal/error flushes produce `completed`/`disposed`/`error`. This closed the old gap where overwriting a key silently dropped the previous payload.
- **High-level `onSchemas` grew real structure:** The high-level Query API still exposes a single `onSchemas` method, but it now accepts richer table entries per schema: `{ field = "id", bufferSize = 10, distinct = false }`. The string form (`schema = "fieldName"`) still works but is now just shorthand for “field only”; table form is the place where users set per-schema buffer sizes and opt into distinct or non-distinct behavior explicitly. We pass normalized selectors plus per-side buffer size hints down to the core, so the DSL lines up exactly with the low-level mechanics.
- **Distinct as a first-class choice:** Instead of “distinct-ish” behavior emerging from implementation details, we now model it directly: `bufferSize = 1` (or `distinct = true`) means “keep only the newest record per key,” while `bufferSize > 1` (or explicit non-distinct) means “keep a short history of events per key.” Matches fan out to all buffered partners, so larger buffers create more join results, and count windows count entries rather than keys so the trade‑offs are predictable and explicit.
- **Demos and viz kept in sync:** All high-level demos (simple, timeline, window zoom, two circles, two zones) now use the new `onSchemas` table form with explicit fields and bufferSize, so everything users see in the viz exercises the same semantics as the tests. The viz adapter and projection-map specs were updated to infer projection fields from the structured `onSchemas` map, rather than relying on implicit string shortcuts.

### What we learned
1. **Two levers, two questions:** Windowing and buffering solve different problems. The window answers “for how long/how many entries overall should we keep events warm,” while the per-key buffer answers “how many events with the same key should we keep at once.” Writing this down forced us to separate global retention (window) from per-key distinctness (buffer) and made configuration less mysterious.
2. **Every removal deserves a story:** By ensuring that overwrites (`replaced`), window evictions (`evicted`/`expired_*`), and terminal flushes (`completed`/`disposed`/`error`) all emit expire packets, we removed the last silent “black hole” in the cache. This makes reasoning about churn possible: you can now reconstruct exactly how and why a record left the join, which is critical for debugging and for any downstream replay/backfill logic.
3. **High-level sugar should describe intent, not hide it:** Moving distinctness into `bufferSize`/`distinct` on `onSchemas` exposed the real decisions users are making (latest snapshot vs recent history) instead of burying them behind max-cache-size quirks. The same `onSchemas` call now captures both “what fields join on” and “how many per-key events we want to keep,” which fits how people think when they design joins.
4. **Docs and tests shape the API:** Updating the explainer and low-level API docs to talk in terms of “how long can an event wait for a partner” and “how many events per key do we keep” gave us a better vocabulary, which in turn made it much easier to write tests that match intent rather than implementation trivia. The new high-level specs around bufferSize and distinct now act as guardrails so future refactors keep these behaviors stable.
