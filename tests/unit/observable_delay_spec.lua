...
describe("Observable delay sourceTime stamping", function()
    ...
    it("jittered mode applies independent offsets", function()
        local emissions
        withRandomSequence({ 0.0, 1.0 }, function()
            emissions = drainEmissions({ minDelay = 0.1, maxDelay = 0.3, delayMode = "jittered" })
        end)
        assert.are.equal(2, #emissions)
        assert.is_true(emissions[1].time >= (0.1 - DELAY_TOLERANCE) and emissions[1].time <= (0.3 + DELAY_TOLERANCE))
        assert.is_true(emissions[2].time >= (0.1 - DELAY_TOLERANCE) and emissions[2].time <= (0.3 + DELAY_TOLERANCE))
        assert.are_not.equal(emissions[1].time, emissions[2].time)
    end)

    it("ordered mode preserves relative ordering", function()
        local emissions = drainEmissions({ minDelay = 0.1, maxDelay = 0.1, delayMode = "ordered" })
        assert.is_true(math.abs(emissions[1].time - 0.1) < 1e-3)
        assert.is_true(math.abs(emissions[2].time - 0.2) < 1e-3)
    end)
end)
