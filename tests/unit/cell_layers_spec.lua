...
describe("cell_layers helper", function()
	...
	it("blends simultaneous layers deterministically (oldest first)", function()
		local layer = CellLayers.CellLayer.new({ 0, 0, 0, 1 }, 1)
		layer:addLayer({ color = { 1, 0, 0, 1 }, ts = 0.1, ttl = 1, id = "red" })
		layer:addLayer({ color = { 0, 1, 0, 1 }, ts = 0, ttl = 1, id = "green" })

		layer:update(0.2)
		local color = select(1, layer:getColor())
		assert.is_true(color[1] > 0.4 and color[1] < 0.7)
		assert.is_true(color[2] > 0.3 and color[2] < 0.6)
	end)
end)
