...
describe("viz_high_level headless renderer", function()
	it("maps inner and layered borders to grid cells", function()
		...
		assert.are.same(1, snap.meta.sourceCounts.customers)
		assert.are.same(1, snap.meta.matchCount)
		assert.are.same(1, snap.meta.expireCount)
		assert.are.same(snap.window, snap.meta.header.window)
		assert.is_table(snap.meta.header.joins or {})
		assert.is_true(#snap.meta.legend >= 1)
		assert.are.same("customers", snap.meta.legend[1].schema)
		assert.are.same(1, snap.meta.legend[1].count)
		assert.are.same(2, #snap.meta.outerLegend)
