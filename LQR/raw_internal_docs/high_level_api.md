...
- `:on{ schemaA = "id", schemaB = "orderId", ... }` → explicit map for join keys. Required.
  - Per-entry options: `{ field = "...", oneShot = true }`:
    - causes matched records on that side to be consumed on match, expiring them with reason
      ...
      one row per key while still consuming each row after its first match.
- `:joinWindow{ time=seconds | count=n, field="sourceTime", currentFn=os.time, gcIntervalSeconds? }`
  ...
  `:joinWindow`.
- `:distinct(schema, { by = "field" | fn, window = { time | count, field?, currentFn?, gcIntervalSeconds?, gcOnInsert? } })`
  → per-schema deduplication operator that runs on the current stream (even after earlier joins),
  keeping at most one record per `by` key inside the configured window. Non-target schemas pass through unchanged.
