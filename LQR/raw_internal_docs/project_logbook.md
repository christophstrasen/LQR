...
- **Metrics snapshots split into “light” vs “full”:** Added `buffer:metrics_getLight()` for hot paths (safe to call frequently) and `buffer:metrics_getFull()` for detailed inspection. The scheduler now prefers light snapshots to avoid monitoring becoming the bottleneck.
