...
**Important:** count mode is a **global cache cap per side**, not a per-key limit. Entries are evicted in insertion order when the side’s cache exceeds `maxItems`, so the way the window “slides” depends on key uniqueness and arrival patterns: a high-cardinality stream churns many different keys through the cache, while a low-cardinality stream with small per-key buffers may rarely hit the global cap.
